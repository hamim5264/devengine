...
export default function CommercialLicensePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        console.warn("Using fallback license config:", err);
      }
    }
    load();
    return () => {
  ...
  const handleCopyLink = (sectionId: string) => {
    const url = `${window.location.origin}/commercial-license#${sectionId}`;
  ...
  const allSections: { key: keyof LicenseConfig; section: LicenseSection }[] = [
    { key: "grantOfLicense", section: config.grantOfLicense },
    { key: "permittedUses", section: config.permittedUses },
    { key: "restrictions", section: config.restrictions },
    { key: "sourceCodeRights", section: config.sourceCodeRights },
    { key: "intellectualProperty", section: config.intellectualProperty },
    { key: "warrantiesAndLiability", section: config.warrantiesAndLiability },
    { key: "termination", section: config.termination },
    { key: "auditAndCompliance", section: config.auditAndCompliance },
  ];
  ...
  return (
    <>
      ...
      <div
        className="fixed top-0 left-0 h-[2px] bg-[#38f2ff] z-50 shadow-[0_0_12px_#38f2ff] transition-all duration-75"
      ...
      <div className="min-h-screen bg-[#02040A] text-[#dde2f3] font-sans selection:bg-[#38f2ff] selection:text-[#02040A]">
        ...
        <div className="fixed -top-40 -left-40 w-[600px] h-[600px] bg-[#38f2ff]/[0.05] rounded-full blur-[140px] pointer-events-none z-[-7]" />
        ...
        <header className="fixed top-0 w-full z-40 bg-[#02040A]/85 backdrop-blur-xl border-b border-white/5 py-4 px-6 sm:px-12 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-lg bg-[#38f2ff]/10 hover:bg-[#38f2ff]/20 border border-[#38f2ff]/30 text-xs font-mono text-[#38f2ff] transition flex items-center gap-1.5"
        ...
        <main className="pt-28 pb-24 px-4 sm:px-8 max-w-7xl mx-auto">
          ...
          <div className="py-12 border-b border-white/10 relative">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <span className="px-3 py-1 rounded-full bg-[#38f2ff]/10 border border-[#38f2ff]/30 text-[#38f2ff] font-mono text-[11px] font-bold uppercase tracking-wider">
              ...
              <span className="text-gray-500 text-xs font-mono">
                Version: {config.header.version} • Last Revised: {config.header.lastUpdated}
              </span>
            </div>
            ...
            <div className="mt-8 max-w-md relative">
              ...
              <input
                ...
                placeholder="Filter license clauses (e.g., resale, SaaS, GitHub)..."
                className="w-full bg-[#08111f] border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-[#38f2ff] focus:ring-2 focus:ring-[#38f2ff]/20 font-mono transition"
              />
              {searchQuery && (
                <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-12">
            ...
            <aside className="lg:col-span-4 hidden lg:block">
              <div className="sticky top-28 bg-[#08111f]/80 border border-white/10 rounded-2xl p-6 backdrop-blur-xl space-y-4">
                <div className="flex items-center gap-2 font-mono text-xs font-bold text-[#38f2ff] uppercase tracking-wider pb-3 border-b border-white/10">
                  <span className="material-symbols-outlined text-base">format_list_bulleted</span>
                  <span>Articles & Clauses</span>
                </div>
                    ...
                    return (
                      <a
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all ${
                          isCurrent
                            ? "bg-[#38f2ff]/10 text-[#38f2ff] font-bold border-l-2 border-[#38f2ff]"
                ...
                <div className="pt-4 border-t border-white/10">
                  <div className="p-3 bg-[#050b14] rounded-xl border border-white/5 text-[11px] text-gray-400 space-y-1">
                    <div className="text-white font-semibold flex items-center gap-1.5">
                      ...
                      <span>Verified Commercial Tier</span>
                    </div>
                    <p className="text-[10px] leading-normal text-gray-400">
                      Standard software licenses are accompanied by a cryptographic Order ID generated upon payment confirmation.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-8 space-y-8">
                  ...
                  >
                    <div className="flex items-start justify-between gap-4 mb-4 pb-3 border-b border-white/5">
                      <div className="flex items-center gap-3">
                        <span className="font-mono text-sm font-bold text-[#38f2ff] px-2.5 py-1 rounded bg-[#38f2ff]/10 border border-[#38f2ff]/20">
                          {section.number}
                        </span>
                      ...
                      <button
                        ...
                        className="text-gray-400 hover:text-[#38f2ff] transition p-1.5 rounded-lg hover:bg-white/5 flex items-center gap-1 text-[11px] font-mono"
                        ...
                        <ul className="space-y-2 pt-2 border-t border-white/5 font-sans">
                          {section.bullets.map((b, bIdx) => (
                            <li key={bIdx} className="flex items-start gap-2.5 text-xs text-gray-300">
                              <span className="material-symbols-outlined text-[#38f2ff] text-base shrink-0 mt-0.5">
                                check_circle
                              </span>
                              <span>{b}</span>
                            </li>
              ...
              {/* Strict No-Return Cross Reference Notice */}
              <div className="p-6 rounded-2xl bg-amber-500/10 border border-amber-500/25 text-amber-200 space-y-3">
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-amber-400 text-xl">gavel</span>
                  <h3 className="font-space font-bold text-base text-amber-300">
                    Cross-Reference: Strict No Return / No Refund Policy
                  </h3>
                </div>
                <p className="text-xs text-amber-200/90 leading-relaxed font-sans">
                  All digital software licenses, architecture blueprints, and source repositories delivered under this agreement are governed by the DevEngine Strict No-Return Policy. Digital goods cannot be refunded once delivered.
                </p>
                <div className="pt-2">
                  <Link
                    href="/refund-policy"
                    className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-amber-300 hover:text-amber-100 underline decoration-amber-400/50 hover:decoration-amber-200"
                  >
                    <span>Read Strict No Return & Refund Policy</span>
        ...
        {showBackToTop && (
          <button
            onClick={scrollToTop}
            className="fixed bottom-8 right-8 z-50 p-3 rounded-xl bg-[#08111f] border border-[#38f2ff]/40 text-[#38f2ff] hover:bg-[#38f2ff] hover:text-[#02040A] shadow-[0_0_20px_rgba(56,242,255,0.3)] transition duration-200 flex items-center justify-center cursor-pointer"
            title="Scroll to Top"
          >
